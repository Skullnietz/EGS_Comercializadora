...
class ModeloCitas
{
	...
	static public function mdlMostrarCitas($tabla, $item, $valor)
	{
		...
		if ($item != null) {
			...
			$stmt = Conexion::conectar()->prepare("SELECT id, title, description, start, end, color, id_orden FROM $tabla WHERE oculto = 0");
			$stmt->execute();
	...
	{
		$stmt = Conexion::conectar()->prepare("SELECT id, title, description, start, end, color, id_orden FROM $tabla WHERE oculto = 0 AND start BETWEEN :inicio AND :fin ORDER BY start ASC");
	...
	static public function mdlVerificarDuplicado($tabla, $fecha, $idOrden)
	{
		...
		$stmt = Conexion::conectar()->prepare(
			"SELECT id, title, start FROM $tabla WHERE oculto = 0 AND id_orden = :idOrden AND start = :fecha LIMIT 1"
		);
		...
		$stmt2 = Conexion::conectar()->prepare(
			"SELECT id, title, start FROM $tabla WHERE oculto = 0 AND id_orden = :idOrden AND DATE(start) = :fecha LIMIT 1"
		);
	...
	static public function mdlMostrarCitasPorOrdenes($tabla, $ordenIds)
	{
		...
		$stmt = Conexion::conectar()->prepare("SELECT id, title, description, start, end, color, id_orden FROM $tabla WHERE oculto = 0 AND id_orden IN ($in)");
		$stmt->execute();
		$citas = $stmt->fetchAll(PDO::FETCH_ASSOC);
		return self::enriquecerCitas($citas);
	}
	...
	static public function mdlCitasPorRangoYOrdenes($tabla, $inicio, $fin, $ordenIds)
	{
		...
		$in = implode(',', array_map('intval', $ordenIds));
		$stmt = Conexion::conectar()->prepare("SELECT id, title, description, start, end, color, id_orden FROM $tabla WHERE oculto = 0 AND start BETWEEN :inicio AND :fin AND id_orden IN ($in) ORDER BY start ASC");
		$stmt->bindParam(":inicio", $inicio, PDO::PARAM_STR);
		$stmt->bindParam(":fin", $fin, PDO::PARAM_STR);
		$stmt->execute();
		$citas = $stmt->fetchAll(PDO::FETCH_ASSOC);
	...
	/*=============================================
	OCULTAR CITA (soft-hide, no se elimina)
	=============================================*/
	static public function mdlOcultarCita($tabla, $datos)
	{
