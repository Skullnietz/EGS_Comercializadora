...
/* ═══════════════════════════════════════════════════════
   MONEDERO ELECTRÓNICO — infoOrden
   Panel visible solo cuando el estado nuevo es "Entregado (Ent)"
   ═══════════════════════════════════════════════════════ */
(function () {
    var $estadoSelect   = $('select[name="estado"]');
    var $panel          = $('#egsMonederoPanel');
    var $montoInput     = $('#egsMontoMonederoOrden');
    var $montoHidden    = $('#egsMontoMonederoOrdenHidden');
    var $btnTodo        = $('#egsMonederoUsarTodo');
    var $desglose       = $('#egsMonederoDesglose');
    var saldoMax        = parseFloat($montoInput.attr('max') || 0);

    function fmt(n) { return '$' + parseFloat(n).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ','); }

    function actualizarDesglose() {
        var totalBruto = parseFloat($('#costoTotalDeOrden').val()) || 0;
        var monto      = Math.min(parseFloat($montoInput.val()) || 0, saldoMax, totalBruto);
        if (monto < 0) monto = 0;

        $montoHidden.val(monto.toFixed(2));

        if (monto > 0) {
            $('#egsMondBruto').text(fmt(totalBruto));
            $('#egsMondDescuento').text('-' + fmt(monto));
            $('#egsMondTotal').text(fmt(Math.max(0, totalBruto - monto)));
            $desglose.show();
        } else {
            $desglose.hide();
            $montoHidden.val('0');
    ...
    // Mostrar/ocultar según estado seleccionado
    $estadoSelect.on('change', function () {
        mostrarPanel($(this).val() === 'Entregado (Ent)');
        if ($(this).val() === 'Entregado (Ent)') actualizarDesglose();
    });

    // Recalcular cuando cambia el monto ingresado
    $montoInput.on('input change', function () {
        var v = parseFloat($(this).val()) || 0;
        if (v > saldoMax) { $(this).val(saldoMax.toFixed(2)); }
        actualizarDesglose();
    });

    // "Usar todo" — aplica el saldo completo limitado al total del servicio
    $btnTodo.on('click', function () {
        var totalBruto = parseFloat($('#costoTotalDeOrden').val()) || 0;
        var aplicar    = Math.min(saldoMax, totalBruto);
        $montoInput.val(aplicar.toFixed(2));
        actualizarDesglose();
    });

    // Recalcular si el total del servicio cambia mientras el panel está abierto
    $('#costoTotalDeOrden').on('change input', function () {
        if ($panel.hasClass('visible')) actualizarDesglose();
    });
}());
