...
if ($_noti_perfil == "administrador") {
  ...
  if (is_array($_noti_raw)) {
    foreach ($_noti_raw as $o) {
      ...
      if (
        strpos($est, "Ent") === false && strpos($est, "can") === false
...
<li class="dropdown notifications-menu">
  ...
  <ul class="dropdown-menu" style="width:360px">
    ...
    <li class="header"
      style="font-size:12px;font-weight:600;display:flex;align-items:center;justify-content:space-between;padding:10px 14px">
      <span>
        <?php if ($_noti_total > 0): ?>
...
══════════════════════════════════════════ -->
  ...
  <div id="egsToast" style="display:none">
    ...
    <div style="padding:14px 16px;display:flex;align-items:flex-start;gap:12px">
      ...
      <!-- Icono -->
      <div
        style="width:40px;height:40px;border-radius:12px;background:<?php echo $_toast_color[1]; ?>;display:flex;align-items:center;justify-content:center;flex-shrink:0;border:2px solid <?php echo $_toast_color[0]; ?>25">
        <i class="fa-solid <?php echo $_toast_color[2]; ?>"
          style="font-size:16px;color:<?php echo $_toast_color[0]; ?>"></i>
      </div>
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:12px;color:#475569;line-height:1.5">
          ...
          <?php else: ?>
            ...
            cambió a
            <strong
              style="color:<?php echo $_toast_color[0]; ?>"><?php echo htmlspecialchars($_toast_first['estado_nuevo']); ?></strong>
          <?php endif; ?>
      ...
      <!-- Cerrar -->
      <button type="button"
        onclick="document.getElementById('egsToast').classList.add('egs-toast-hide');setTimeout(function(){document.getElementById('egsToast').style.display='none'},400)"
...
══════════════════════════════════════════ -->
  <style>
    #egsToastObs {
      position: fixed;
      bottom:
        <?php echo ($_noti_totalEstado > 0) ? '110px' : '24px'; ?>
      ;
      right: 24px;
      z-index: 99998;
      width: 370px;
  ...
  <div id="egsToastObs" style="display:none">
    ...
    <div style="padding:14px 16px;display:flex;align-items:flex-start;gap:12px">
      ...
      <div
        style="width:40px;height:40px;border-radius:12px;background:#fffbeb;display:flex;align-items:center;justify-content:center;flex-shrink:0;border:2px solid rgba(245,158,11,.25)">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:11px;color:#94a3b8;margin-top:2px">
          <i class="fa-solid fa-user"
            style="font-size:8px;margin-right:2px"></i><?php echo htmlspecialchars($_toastObs_creador); ?>
        </div>
      ...
      <button type="button"
        onclick="document.getElementById('egsToastObs').classList.add('egs-toast-hide');setTimeout(function(){document.getElementById('egsToastObs').style.display='none'},400)"
        style="border:none;background:none;color:#94a3b8;font-size:14px;cursor:pointer;padding:0;line-height:1;flex-shrink:0;margin-top:-2px"
        title="Cerrar">
        <i class="fa-solid fa-xmark"></i>
      </button>
...
// No existía el <ul class="menu">, crear la estructura
  ...
  var $noItems = $dropdownUl.find('li.footer');
  $('<li>
    <ul class="menu" style="max-height:400px;overflow-y:auto"></ul>
  </li>').insertBefore($noItems);
  var $newMenu = $dropdownUl.find('.menu');
  $newMenu.append(egsBuildDropdownItem({
  type: r.ultimaNotif.tipo || 'estado',
  idOrden: r.ultimaNotif.idOrden,
