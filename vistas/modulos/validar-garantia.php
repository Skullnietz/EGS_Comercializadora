...
$empresaGarantia = is_array($garantia) && !empty($garantia["id_empresa"]) ? intval($garantia["id_empresa"]) : 1;
...
<main class="egs-warranty-public">
  <section class="egs-warranty-card">
    <header class="egs-warranty-brand">
      <img src="vistas/img/plantilla/logo.png" alt="EGS">
      <div><b><?= egsGarantiaH($configGarantia["nombre_comercial"]) ?></b><small>Validación oficial de garantía</small></div>
    </header>
    ...
    <div class="egs-warranty-status <?= egsGarantiaH($estadoGarantia) ?>">
      ...
      <div><span>ESTADO DE COBERTURA</span><h1><?= egsGarantiaH($tituloGarantia) ?></h1><p><?= egsGarantiaH($detalleGarantia) ?></p></div>
    </div>
