...
if($_SESSION["perfil"] != "administrador"  AND $_SESSION["perfil"]!= "vendedor" AND $_SESSION["perfil"]!= "secretaria" AND $_SESSION["perfil"]!= "Super-Administrador" AND $_SESSION["perfil"]!= "tecnico"){
...
<script>
 $(document).ready(function(){
	...
	var _puedeImprimirEtiquetaContacto = <?php echo isset($_SESSION["perfil"]) && $_SESSION["perfil"] === "administrador" ? "true" : "false"; ?>;
    ...
    "columns":[
        ...
        {data: null,
            "render": function (data, type, row, meta ) {
                ...
                return "<span style='display:inline-flex;align-items:flex-start;flex-wrap:wrap;gap:4px;max-width:100%;white-space:normal;line-height:1.35'>"
                     + "<span style='display:inline-block;white-space:normal;word-break:break-word;min-width:180px' title='"+nombre+"'>"+nombre+"</span>"
                     + badges
                     + "</span>";
            }},
        {data: null,
        ...
        "render": function (data, type, row, meta) {
            ...
            var botonGarantia = esEntregada
                ? ' <a href="index.php?ruta=imprimir-etiqueta-orden&tipo=garantia&idOrden='+data.id+'" class="btn btn-success btnImprimirEtiquetaGarantia" idOrden="'+data.id+'" title="Imprimir etiqueta de garantía" aria-label="Imprimir etiqueta de garantía"><span class="fa fa-shield-halved"></span></a>'
                : '';
            return '<a type="button" href="/extensiones/tcpdf/pdf/ticketOrden.php/?idOrden='+ data.id +'&empresa='+ data.id_empresa +'&asesor='+ data.id_Asesor +'&cliente='+ data.id_usuario +'&tecnico='+ data.id_tecnico+'" id="ButtonImprimir" class="editar btn btn-primary botonImprimir btnImprimirorden" idOrden="'+data.id+'" empresa="'+data.id_empresa+'" asesor="'+data.id_Asesor+'" cliente="'+data.id_usuario+'" tecnico="'+data.id_tecnico+'" estado="'+estadoOrden.replace(/"/g, '&quot;')+'" data-etiqueta-garantia="'+(esEntregada ? '1' : '0')+'" title="'+tituloImpresion+'" aria-label="'+tituloImpresion+'" target="'+destinoTicket+'"><span class="fa fa-print"></span></a>'+botonEtiqueta+botonGarantia;
      ...
      <div class="modal-body" style="padding:20px 24px">
        <div class="box-body" style="padding:0">
          ...
          <div class="egs-section">
            <div class="egs-section-title"><i class="fa-solid fa-users"></i> Asignación</div>
            <div class="row egs-row">
              ...
              <div class="col-md-6">
                <div class="form-group">
                  <label class="egs-lbl"><i class="fa-solid fa-screwdriver-wrench" style="margin-right:4px"></i>Técnico<span class="egs-req">*</span></label>
          ...
          <div class="egs-section">
            ...
            <div class="row egs-row">
              <div class="col-md-6">
                <div class="form-group">
                  <label class="egs-lbl"><i class="fa-solid fa-barcode" style="margin-right:4px"></i>Número de Serie<span class="egs-req">*</span> <span style="font-weight:400;color:#94a3b8">(últimos 6 dígitos)</span></label>
          ...
          <!-- SECCION: Evidencia -->
          <div class="egs-section">
            <div class="egs-section-title"><i class="fa-solid fa-camera"></i> Evidencia Fotográfica</div>
            <div class="form-group agregarMultimedia">
              <div class="multimediaOrden needsclick dz-clickable">
                <div class="dz-message needsclick cuadro multimedia" style="border:2px dashed #e2e8f0;border-radius:10px;padding:24px;text-align:center;background:#fafbfc;transition:all .2s;cursor:pointer">
                  ...
                  <span style="font-size:13px;font-weight:600;color:#64748b">Arrastrar o dar clic para subir imágenes</span>
                  <br><span style="font-size:11px;color:#94a3b8">JPG o PNG &bull; máx. 2MB por imagen &bull; hasta 10 fotos</span>
                </div>
              </div>
            </div>
