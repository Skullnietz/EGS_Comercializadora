...
class ControladorSlide{
	...
	static public function ctrActualizarSlide($datos){
		...
		if($datos["subirFondo"] != null){
			...
			if($datos["subirFondo"]["type"] == "image/gif"){

				$rutaGif = $directorio."/fondo.gif";

				$origen = imagecreatefromgif($datos["subirFondo"]["tmp_name"]);

				imagecopyresized($destino, $origen, 0, 0, 0, 0, $nuevoAncho, $nuevoAlto, $ancho, $alto);

				imagegif($destino, $rutaGif);

				//imagegif ( resource $image [, string $rutaGif ] ) : bool

				$respuesta = ModeloSlide::MdlModificarSlideConGif($tabla, $rutaGif, $datos);
		
			}


			$rutaFondo = substr($rutaGif, 3);
			...
			if($respuesta == "ok"){
				...
				swal({
					  ...
					  confirmButtonText: "Cerrar",		
								...
								window.location = "index.php?ruta=slide";
