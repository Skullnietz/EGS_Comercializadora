...
class TablaOrdenes{
  ...
  public function mostrarTabla(){	
	...
	for($i = 0; $i < count($pedidos); $i++){
		...
		$estado = isset($pedidos[$i]["estado"]) ? $pedidos[$i]["estado"] : '';
		...
		$acciones = "<div class='btn-group' style='gap:4px;'>"
			. "<button class='btn btn-sm btn-danger btnEliminarPedido' idPedido='".$pedidos[$i]["id"]."' title='Eliminar' style='padding:6px 10px;'><i class='fa fa-trash'></i></button>"
			. "<button class='btn btn-sm btn-warning btnImprimirTicketPedido' idPedido='".$pedidos[$i]["id"]."' cliente='".$pedidos[$i]["id_cliente"]."' asesor='".$pedidos[$i]["id_Asesor"]."' empresa='".$pedidos[$i]["id_empresa"]."' title='Imprimir' style='padding:6px 10px;'><i class='fas fa-print'></i></button>"
			. "</div>";
		...
		$NombreEmpresa = is_array($respuesta) && isset($respuesta["empresa"]) ? $respuesta["empresa"] : '';
		...
		$ordenHtml = (!empty($pedidos[$i]["id_orden"]) && $pedidos[$i]["id_orden"] != '0')
			? "<span style='color:#6366f1;font-weight:600'>#".$pedidos[$i]["id_orden"]."</span>"
