...
class TablaVentas{
  ...
  public function mostrarTabla(){	
	...
	for($i = 0; $i < count($ventas); $i++){
		...
		$imgProducto = "<img loading='lazy' class='img-thumbnail' src='".$traerProducto[0]["portada"]."' width='100px'>";
		...
		if($traerCliente["foto"] != ""){
			...
			$imgCliente = "<img loading='lazy' class='img-circle' src='".$traerCliente["foto"]."' width='70px'>";

		}else{

			$imgCliente = "<img loading='lazy' class='img-circle' src='vistas/img/usuarios/default/anonymous.png' width='70px'>";
		}
		...
		if($ventas[$i]["metodo"] == "paypal"){
			...
			$metodo = "<img loading='lazy' class='img-responsive' src='vistas/img/plantilla/paypal.jpg' width='300px'>";
		
		}else if($ventas[$i]["metodo"] == "payu"){

			$metodo = "<img loading='lazy' class='img-responsive' src='vistas/img/plantilla/payu.jpg' width='300px'>";
		...
		$pdfComprovanteDeVenta = "<div class='btn-group'><button class='btn btn-warning btnImprimirpdfComprovanteDeVenta' idventa='".$ventas[$i]["id"]."' data-toggle='modal' data-target='#modalImprimirTiket'><i class='fa fa-file'></i></button></div>";

		$ticketComprovanteDeVentaTicket = "<div class='btn-group'><button class='btn btn-warning btnImprimirTiket' idventa='".$ventas[$i]["id"]."' data-toggle='modal' data-target='#modalImprimirTiket'><i class='fa fa-ticket'></i></button></div>";
		/*=============================================
		DEVOLVER DATOS JSON
		=============================================*/
		$datosJson	 .= '[
			      		...
			      		"'.$ventas[$i]["pais"].'",
			      		"'.$ventas[$i]["fecha"].'",
			      		"'.$pdfComprovanteDeVenta.'",
			      		"'.$ticketComprovanteDeVentaTicket.'"	
			      		],';
